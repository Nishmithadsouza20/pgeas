...
def reset_owner_password(cid):
    ...
    db.commit()
    db.close()
    print(f"\n[EMAIL] Password reset for {company['owner_email']} — new password: {new_password}\n")
    return jsonify({'message': f"Password reset. New password: {new_password}"})
...
def provision_client():
    ...
    print(f"Password: {data['owner_password']}")
